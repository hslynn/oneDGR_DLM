...
def main():
    ...
    with open(folder+'run.sh', 'w') as f:
        f.write('#!/bin/bash\n')
        f.write('cd ~/projects/oneDGR_DLM/\n')
        f.write('python3 oneDGR_DLM.py ')
    ...
    r = project(Expression('x[0]', degree=10), func_space)
    ...
    form_packs = (invg_forms, auxi_forms, gamma_forms, C_forms, src_forms) 
    func_packs = (invg_list, auxi_list, gamma_list, C_list, src_list)
    ...
    temp_list = [Function(func_space) for dummy in range(N)] 
    rhs_list = [Function(func_space) for dummy in range(N)]

    #define variational forms
    u1 = TrialFunction(func_space)
    v = TestFunction(func_space)
    variational_forms = variational.get_variational_forms(u1, v, var_list, auxi_list, src_list, exact_var_list)
    ...
    fig_error_sub = plt.figure(figsize=(19.2, 10.8))
    ...
    while t < t_end:
        ...
        if DG_degree == 1:
            rk.rk3(var_list, temp_list, rhs_list, form_packs, func_packs, variational_forms, dt)
        else:
            rk.rk3(var_list, temp_list, rhs_list, form_packs, func_packs, variational_forms, dt)

        #for var in var_list:
        #    limiter.apply_minmod_limiter(var)
        ...
        if time.time() - last_save_wall_time > 200 or t == t_end:
            #save data to files    
            for idx in range(len(var_list)): 
                var = var_list[idx]
            ...
            #error near horizon
            fig_error_sub.clf()
            fig_error_sub.subplots(3, 3)
            fig_error_sub.suptitle('error when t = '+str(t)+", near horizon")
            for idx in range(9):
                plot_obj = fig_error_sub.axes[idx]
                ioo.plot_function_dif(var_list[idx], exact_var_list[idx], inner_bdry, 2.5, plot_obj)
                plot_obj.set_title('error of '+str(var_list[idx].name()))
            fig_error_sub.savefig(folder+'sub_error_'+t_str+'.png')
